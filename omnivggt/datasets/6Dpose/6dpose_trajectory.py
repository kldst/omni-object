...
class SixDPose(BaseStereoViewDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.layout = self._detect_layout(dataset_location)
        self.scene_root, self.depth_root, self.cam_root, self.pose_root = self._resolve_layout_roots(dataset_location)
        ...
        explicit_scene_name = (only_scene_name or "").strip()
        explicit_scene_names = [x.strip() for x in (only_scene_names or []) if str(x).strip()]
        explicit_scene_start = only_scene_start
        explicit_scene_end = only_scene_end

        if not explicit_scene_name and not explicit_scene_names and explicit_scene_start is None and explicit_scene_end is None:
            explicit_scene_name = (only_run_name or "").strip()
            explicit_scene_names = [x.strip() for x in (only_run_names or []) if str(x).strip()]
            explicit_scene_start = only_run_start
            explicit_scene_end = only_run_end

        self.only_scene_name = explicit_scene_name
        self.only_scene_names = explicit_scene_names
        if self.only_scene_name:
            self.only_scene_names = [self.only_scene_name]

        split_scene_start, split_scene_end = self._default_split_run_range(self.dset)
        self.only_scene_start_idx = (
            self._parse_sequence_index(explicit_scene_start) if explicit_scene_start is not None else split_scene_start
        )
        self.only_scene_end_idx = (
            self._parse_sequence_index(explicit_scene_end) if explicit_scene_end is not None else split_scene_end
        )
    ...
    @staticmethod
    def _detect_layout(dataset_location: str) -> str:
        if osp.isdir(osp.join(dataset_location, "out_image")):
            return "run"
        if any(entry.startswith("scene_") and osp.isdir(osp.join(dataset_location, entry)) for entry in os.listdir(dataset_location)):
            return "scene"
        raise FileNotFoundError(
            "Unable to infer dataset layout. Expected either root/out_image or root/scene_*/out_image."
        )

    def _resolve_layout_roots(self, dataset_location: str):
        if self.layout == "run":
            return (
                osp.join(dataset_location, "out_image"),
                osp.join(dataset_location, "out_depth"),
                osp.join(dataset_location, "out_cam_param"),
                osp.join(dataset_location, "out_pose"),
            )
        return dataset_location, dataset_location, dataset_location, dataset_location
    ...
    @staticmethod
    def _parse_sequence_index(sequence_name: Optional[str]) -> Optional[int]:
        if sequence_name is None:
            return None
        value = str(sequence_name).strip()
        if not value:
            return None
        if value.isdigit():
            return int(value)
        ...
        if value.startswith("scene_") and value[6:].isdigit():
            return int(value[6:])
        raise ValueError(f"Invalid sequence spec '{sequence_name}'. Expected e.g. 'run_0000', 'scene_0000', or '0'.")
    ...
    def _filter_sequence_names(self, sequence_names: List[str]) -> List[str]:
        ...
        if self.only_scene_names:
            ...
            for sequence_name in self.only_scene_names:
                ...
                sequence_idx = self._parse_sequence_index(sequence_name)
                if sequence_idx is not None:
                    normalized_scene_names.add(f"scene_{sequence_idx:04d}")
                    normalized_scene_names.add(f"run_{sequence_idx:04d}")
    ...
    def _list_run_names(self) -> List[str]:
        ...
        return self._filter_sequence_names(run_names)
    ...
    def _list_scene_frame_names(self, scene_name: str) -> List[str]:
        pose_dir = osp.join(self.dataset_location, scene_name, "out_pose")
        if not osp.isdir(pose_dir):
            return []
        frame_names = []
        for file_name in os.listdir(pose_dir):
            if not file_name.endswith(".npz") or not file_name.startswith("frame_"):
                continue
            frame_name = osp.splitext(file_name)[0]
    ...
    def __getitem__(self, idx):
        ...
        if record.get("layout") == "scene":
            scene_view_indices = [0] * max(1, num_views)
            views = [
                self._load_scene_frame_view(record["scene_name"], record["frame_name"], resolution, self._rng)
                for _ in scene_view_indices
            ]
        else:
