...
class OO9DCameraPoseBase(BaseStereoViewDataset):
    """Base class for OV9D-style camera-pose datasets in the OO9D package."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dataset_label = type(self).__name__
        ...
        if not self.records:
            raise RuntimeError(
                f"No {self.dataset_label} samples found. split_json={self.split_json}, "
                f"multi_root={self.multi_root}, single_root={self.single_root}"
            )
        logger.info(
            "%s initialized: dset=%s records=%d object_presence_prob=%.3f fixed_object_view_ids=%s "
            "strict_fixed_object_view_ids=%s",
            self.dataset_label,
            self.dset,
            len(self.records),
            ...
            if needed > 0:
                if len(fallback_ids) < needed:
                    raise RuntimeError(
                        ...
                        f"fixed={self.fixed_object_view_ids}, available={available_ids}, need={self.num_object_views}"
                    )
                extra_ids = rng.choice(np.asarray(fallback_ids), size=needed, replace=False).tolist() if self.training else fallback_ids[:needed]
    ...
    def __getitem__(self, idx):
        ...
        result = {
            ...
            "seq_name": (
                f"oo9d_camera_pose/{rec.get('scene_source', 'scene')}/{rec['scene_name']}/"
                f"obj_{target_object_id:06d}/{image_id:06d}"
            ),
            "scene_name": rec["scene_name"],
            "scene_source": rec.get("scene_source", ""),
