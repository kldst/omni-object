...
def _rot6d_candidate_loss(pred: torch.Tensor, candidates: torch.Tensor, loss_type: str) -> torch.Tensor:
    pred = pred.unsqueeze(0).expand_as(candidates)
    if loss_type == "l1":
        return (pred - candidates).abs().mean(dim=-1)
    if loss_type == "l2":
        return ((pred - candidates) ** 2).mean(dim=-1)
    raise ValueError(f"Unknown loss_type: {loss_type}")


@lru_cache(maxsize=8)
def _load_symmetry_info(path: str, continuous_steps: int):
    if not path:
        return {}
...
def _symmetric_rot6d_loss(
    ...
    if not symmetry_info_path:
        return _vector_loss(pred_pose, _rotation_matrix_to_rot6d(gt_rot), loss_type=loss_type)
    ...
    for sample_idx, object_id in enumerate(object_ids_cpu):
        ...
        if sym_rots is None:
            ...
            sym_rots = sym_rots.to(device=gt_rot.device, dtype=gt_rot.dtype)
            candidate_rots = torch.matmul(gt_rot[sample_idx].unsqueeze(0), sym_rots)
            candidates = _rotation_matrix_to_rot6d(candidate_rots)
        losses.append(_rot6d_candidate_loss(pred_pose[sample_idx], candidates, loss_type).min())
    if not losses:
        return (pred_pose * 0).mean()
    ...
    **kwargs,
    ...
    if pose_rep == "symmetric_rot6d":
        if object_ids is None:
            ...
            loss_pose = _symmetric_rot6d_loss(
                pred_pose,
                gt_rot,
                object_ids,
                dataset_labels,
        ...
        raise ValueError(f"Unknown object pose_rep: {pose_rep}")
    loss_translation = _vector_loss(pred_translation, gt_translation, loss_type=loss_type)
    if pred_size_log is not None and gt_size_log is not None and float(weight_size) != 0.0:
        loss_size = _vector_loss(pred_size_log, gt_size_log, loss_type=loss_type)
    else:
        loss_size = (pred_pose * 0).mean()

    total = weight_pose * loss_pose + weight_translation * loss_translation + weight_size * loss_size
...
def compute_object_mask_loss(
    ...
    **kwargs,
    ...
    if gt_mask.shape != pred_logits.shape:
        raise ValueError(f"object mask shape mismatch: pred={tuple(pred_logits.shape)} gt={tuple(gt_mask.shape)}")

    has_object = batch.get("has_object", None)
    if has_object is not None:
        valid_mask = has_object.to(device=pred_logits.device).bool()
        if valid_mask.sum() == 0:
            dummy = (pred_logits * 0).mean()
            return {
                "loss_object_mask": dummy,
...
def compute_object_presence_loss(
    ...
    **kwargs,
    ...
    targets = batch["has_object"].to(device=logits.device, dtype=logits.dtype).reshape(-1)
    bce_kwargs = {}
    if pos_weight is not None:
        bce_kwargs["pos_weight"] = torch.as_tensor(float(pos_weight), device=logits.device, dtype=logits.dtype)
