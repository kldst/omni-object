...
# python3 demo_gradio_6dpose_0519.py --port 7860 --multi-scene-test
import argparse
import inspect
import json
import re
...
# python demo_gradio_6dpose_0519.py \
#   --google-oo3d-test \
#   --google-oo3d-root /mnt/train-data-4-hdd/yian/freepose/ov9d/combined_renders_upright \
#   --port 7860
#  python demo_gradio_6dpose_0519.py   --google-oo3d-test   --google-oo3d-root /mnt/train-data-4-hdd/yian/freepose/ov9d/combined_renders_mm   --port 7868
...
DEFAULT_EVAL_SPLIT_JSON = (
    PROJECT_ROOT
    / "splits_ov9d_seen_unseen_scene"
    / "single"
    / "val_same_category_unseen_object.json"
)
DEFAULT_TRAIN_SINGLE_SPLIT_JSON = PROJECT_ROOT / "splits_ov9d_seen_unseen_scene" / "single" / "train.json"
DEFAULT_MULTI_SCENE_TEST_SPLIT_JSON = (
    PROJECT_ROOT
    / "splits_ov9d_seen_unseen_scene"
    / "multi"
    / "test_same_category_unseen_object_unseen_scene.json"
)
...
DEFAULT_FREEPOSE_OBJ_ROOT = Path("/mnt/train-data-4-hdd/yian/freepose/obj")
DEFAULT_FIXEDCAM_OBJECT_IMAGE_ROOT = Path("/mnt/train-data-4-hdd/yian/freepose/object_space_renders_all")
DEFAULT_MATCHING_CASES_ROOT = PROJECT_ROOT / "outputs" / "0515_LARGE" / "metric" / "matching_cases"
DEFAULT_GOOGLE_OO3D_TEST_ROOT = Path("/mnt/train-data-4-hdd/yian/freepose/ov9d/google_oo3d9dsingle_test_wall")
...
def resolve_runtime_settings(cfg: Dict) -> Dict:
    ...
    return {
        ...
        "split_json": parse_dataset_ctor_arg(
            dataset_expr,
            "split_json",
            default=parse_dataset_ctor_arg(dataset_expr, "multi_split_json", default=None),
        ),
        "single_split_json": parse_dataset_ctor_arg(dataset_expr, "single_split_json", default=None),
        ...
        "train_single_split_json": parse_dataset_ctor_arg(train_dataset_expr, "single_split_json", default=None),
...
def load_ov9d_object_tensor(
    ...
    for image_id in object_views:
        ...
        if not image_path.is_file():
            raise FileNotFoundError(f"Missing OO9D object reference view: {image_path}")
...
class DemoApp:
    def __init__(
        ...
        dataset_root: Path,
        eval_split_json: Path | None = None,
        train_split_json: Path | None = None,
        use_gt_pose_for_prediction: bool = False,
        google_oo3d_test_root: Path | None = None,
    ):
        self.config_path = Path(config_path)
        self.cfg = load_config(self.config_path)
        self.runtime = resolve_runtime_settings(self.cfg)
        runtime_dataset_root = self.runtime.get("dataset_location")
        self.dataset_root = resolve_local_path(runtime_dataset_root, default=dataset_root) or Path(dataset_root)
        self.object_image_root = (
            resolve_local_path(self.runtime.get("object_image_root"), default=DEFAULT_OBJECT_IMAGE_ROOT)
            or DEFAULT_OBJECT_IMAGE_ROOT
        )
        ...
        self.google_oo3d_test_root = Path(google_oo3d_test_root).expanduser() if google_oo3d_test_root else None

        self.ov9d_multi_root = self.dataset_root / "oo3d9dmulti"
        self.ov9d_single_root = self.dataset_root / "oo3d9dsingle"
        self.ov9d_split_json = resolve_local_path(eval_split_json, default=DEFAULT_EVAL_SPLIT_JSON) or DEFAULT_EVAL_SPLIT_JSON
        self.ov9d_train_split_json = (
            resolve_local_path(train_split_json)
            if train_split_json is not None
            else resolve_local_path(
                self.runtime.get("train_single_split_json") or self.runtime.get("train_split_json"),
            )
        )

        if self.google_oo3d_test_root is not None:
            self._init_google_oo3d_metadata()
        else:
    ...
    # ----- OV9D metadata -----
    def _init_google_oo3d_metadata(self):
        if self.google_oo3d_test_root is None or not self.google_oo3d_test_root.is_dir():
            raise FileNotFoundError(f"Google OO3D test root not found: {self.google_oo3d_test_root}")

        self.dataset_root = self.google_oo3d_test_root
        self.object_image_root = self.google_oo3d_test_root
        self.ov9d_multi_root = self.google_oo3d_test_root
        self.ov9d_single_root = self.google_oo3d_test_root
        self.ov9d_split_json = None
        self.ov9d_train_split_json = None
        self.default_split_name = "google"
        self.ov9d_models_info: Dict[str, Dict] = {}
        self.ov9d_name_to_oid: Dict[str, int] = {}
        self.ov9d_oid_to_name: Dict[int, str] = {}
        self.ov9d_split_records = {"google": {}}
        self.ov9d_all_scene_records = {}
        self.ov9d_single_records_by_object_id: Dict[int, List[Dict]] = {}
        self.ov9d_train_object_ids = set()

        for scene_dir in sorted(p for p in self.google_oo3d_test_root.iterdir() if p.is_dir()):
            required = ["scene_gt.json", "scene_camera.json", "scene_meta.json"]
            ...
            if not image_ids:
                continue
            first_key = str(image_ids[0]) if str(image_ids[0]) in scene_gt else next(iter(scene_gt))
            first_gt = scene_gt[first_key][0]
            object_id = int(first_gt.get("obj_id", 1))
            metadata_path = scene_dir / "metadata.json"
            metadata = read_json(metadata_path) if metadata_path.is_file() else {}
            object_name = str(metadata.get("obj_name") or scene_dir.name.replace("google_", "", 1))
            object_ref_dir = Path(metadata.get("object_reference_dir") or "")
            if not object_ref_dir.is_dir():
                object_ref_dir = scene_dir

            meta_key = first_key if first_key in scene_meta else next(iter(scene_meta))
            model_info = dict(scene_meta[meta_key][0])
            self.ov9d_models_info[str(object_id)] = model_info
            self.ov9d_name_to_oid[object_name] = object_id
            self.ov9d_oid_to_name[object_id] = object_name

            record = {
                "scene_name": scene_dir.name,
                "scene_dir": scene_dir,
                "relative_path": scene_dir.name,
                "object_id": object_id,
                "object_ids": [object_id],
                "eligible_object_ids": [object_id],
                "google_oo3d_test": True,
            }
            self.ov9d_split_records["google"][scene_dir.name] = record
            self.ov9d_all_scene_records[scene_dir.name] = record
            self.ov9d_single_records_by_object_id.setdefault(object_id, []).append(
                {
                    "scene_dir": object_ref_dir,
                    "scene_name": object_ref_dir.name,
                    "image_ids": sorted(int(path.stem) for path in (object_ref_dir / "rgb").glob("*.png")),
        ...
        print(
            f"[demo] Google OO3D test metadata: scenes={len(self.ov9d_split_records['google'])} "
            f"single_objects={len(self.ov9d_single_records_by_object_id)} root={self.google_oo3d_test_root}"
        )
    ...
    def _collect_ov9d_train_object_ids(self) -> set:
        ...
        object_ids.update(int(item["object_id"]) for item in payload.get("scenes", []) if "object_id" in item)
        ...
        if self.object_image_root.is_dir():
            for object_dir in sorted(p for p in self.object_image_root.iterdir() if p.is_dir()):
                try:
                    ...
                    continue
                image_ids = []
                for rgb_path in sorted((object_dir / "rgb").glob("*.png")):
                    image_ids.append(int(rgb_path.stem))
    ...
    def run_inference(
        ...
        (scene_tensor, depth_tensor, mask_tensor,
         display_image, display_depth, gt_mask, intrinsic) = load_ov9d_scene_frame_inputs(
            scene_dir.parent, scene_dir.name, image_id, object_id, self.resolution, self.device,
        )
        ...
        rot_error_deg, sym_count = symmetric_rotation_error_degrees(
            ...
            "" if self.google_oo3d_test_root is not None else str(resolve_local_path(self.cfg.get("object_srt_symmetry_info_path", "")) or ""),
...
def build_demo(app: DemoApp, image_focused_layout: bool = False):
    ...
    info_markdown = "\n".join(
        [
            "# OmniVGGT 6D Pose Demo (OO9D, single-view)",
            ...
            f"- eval split: `{app.ov9d_split_json}`",
            f"- train split: `{app.ov9d_train_split_json}` {anchor_note}",
    ...
    with gr.Blocks(title="OmniVGGT 6D Pose Demo (OO9D)", css=demo_css) as demo:
        if image_focused_layout:
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Gradio demo for OmniVGGT 6D pose inference on OO9D/OV9D")
    ...
    parser.add_argument(
        "--co3d",
        action="store_true",
        help="Use the CO3D web UI and infer frames from the CO3D dataset.",
    )
    parser.add_argument("--co3d-root", type=Path, default=DEFAULT_CO3D_ROOT)
    parser.add_argument(
        "--fixedcam",
        action="store_true",
        help="Use the fixed-camera FreePose/Google dataset web UI.",
    )
    parser.add_argument("--fixedcam-root", type=Path, default=DEFAULT_FIXEDCAM_ROOT)
    parser.add_argument("--obj-root", type=Path, default=DEFAULT_FREEPOSE_OBJ_ROOT)
    parser.add_argument("--fixedcam-object-image-root", type=Path, default=DEFAULT_FIXEDCAM_OBJECT_IMAGE_ROOT)
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Camera view index used to compute each fixedcam run's depth median filter.",
    )
    parser.add_argument(
        "--eval-split-json",
        type=Path,
        default=DEFAULT_EVAL_SPLIT_JSON,
        help="OO9D/OV9D split JSON shown in the web UI.",
    )
    parser.add_argument(
        "--multi-scene-test",
        action="store_true",
        help=(
            "Use the multi-scene test split: "
            f"{DEFAULT_MULTI_SCENE_TEST_SPLIT_JSON}"
        ),
    )
    parser.add_argument(
        "--google-oo3d-test",
        action="store_true",
        help=(
            "Use the synthetic Google OBJ oo3d-style test root instead of OV9D splits: "
            f"{DEFAULT_GOOGLE_OO3D_TEST_ROOT}"
        ),
    )
    parser.add_argument("--google-oo3d-root", type=Path, default=DEFAULT_GOOGLE_OO3D_TEST_ROOT)
    parser.add_argument(
        "--train-split-json",
        type=Path,
        default=None,
    ...
    if args.co3d:
        import demo_gradio_6dpose_co3d as co3d_demo

        co3d_app = co3d_demo.DemoApp(
            args.config,
            args.checkpoint or str(DEFAULT_PRETRAIN_MODEL),
            co3d_root=args.co3d_root,
            ov9d_root=args.dataset_root,
            train_split_json=args.train_split_json or DEFAULT_TRAIN_SINGLE_SPLIT_JSON,
        )
        demo = co3d_demo.build_demo(co3d_app, image_focused_layout=args.image_focused_layout)
        demo.launch(
            server_name=args.host,
            server_port=args.port,
            share=args.share,
            allowed_paths=[
                str(co3d_app.co3d_root),
                str(co3d_app.anchor_index.ov9d_root),
                str(co3d_app.anchor_index.object_image_root),
            ],
        )
        return

    if args.fixedcam:
        import demo_gradio_fixedcam_0519 as fixedcam_demo

        fixedcam_app = fixedcam_demo.FixedCamDemoApp(
            args.config,
            args.checkpoint or str(DEFAULT_PRETRAIN_MODEL),
            data_root=args.fixedcam_root,
            obj_root=args.obj_root,
            object_image_root=args.fixedcam_object_image_root,
        ...
        demo = fixedcam_demo.build_demo(fixedcam_app, image_focused_layout=args.image_focused_layout)
        demo.launch(
            server_name=args.host,
            server_port=args.port,
            share=args.share,
            allowed_paths=[
                str(fixedcam_app.data_root),
                str(fixedcam_app.obj_root),
                str(fixedcam_app.object_image_root),
                *[str(p) for p in fixedcam_app.extra_obj_roots],
            ],
        )
        return

    eval_split_json = DEFAULT_MULTI_SCENE_TEST_SPLIT_JSON if args.multi_scene_test else args.eval_split_json
    app = DemoApp(
        args.config,
        args.checkpoint,
        dataset_root=args.dataset_root,
        eval_split_json=eval_split_json,
        train_split_json=args.train_split_json,
        use_gt_pose_for_prediction=args.use_gt_pose_for_prediction,
        google_oo3d_test_root=args.google_oo3d_root if args.google_oo3d_test else None,
    )
