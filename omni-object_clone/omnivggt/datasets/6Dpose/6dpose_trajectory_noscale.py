...
class SixDPose(BaseStereoViewDataset):
    ...
    def _resolve_scene_frame_image_full_path(self, scene_name: str, frame_name: str) -> str:
        return osp.join(self.dataset_location, self._resolve_scene_frame_image_path(scene_name, frame_name))

    def _resolve_scene_frame_depth_full_path(self, scene_name: str, frame_name: str) -> str:
        return osp.join(self.dataset_location, self._resolve_scene_frame_depth_path(scene_name, frame_name))

    def _resolve_scene_frame_camera_full_path(self, scene_name: str, frame_name: str) -> str:
        return osp.join(self.dataset_location, self._resolve_scene_frame_camera_path(scene_name, frame_name))

    def _resolve_scene_frame_pose_full_path(self, scene_name: str, frame_name: str) -> str:
        return osp.join(self.dataset_location, self._resolve_scene_frame_pose_path(scene_name, frame_name))
    ...
    def _debug_print_sample_paths(self, record: Dict[str, object]) -> None:
        scene_name = str(record["scene_name"])
        frame_name = str(record["frame_name"])
        object_name = str(record["object_name"])

        scene_image_path = self._resolve_scene_frame_image_full_path(scene_name, frame_name)
        depth_path = self._resolve_scene_frame_depth_full_path(scene_name, frame_name)
        camera_path = self._resolve_scene_frame_camera_full_path(scene_name, frame_name)
        pose_path = self._resolve_scene_frame_pose_full_path(scene_name, frame_name)
        object_image_paths = [
            self._resolve_object_image_path(object_name, cam_idx) for cam_idx in self.object_view_pool
        ]

        print(
            "[SixDPose debug] "
            f"scene={scene_name} frame={frame_name} object={object_name}"
        )
        print(f"[SixDPose debug] scene image: {scene_image_path}")
        for cam_idx, object_image_path in zip(self.object_view_pool, object_image_paths):
            print(f"[SixDPose debug] object image view {cam_idx}: {object_image_path}")
        print(f"[SixDPose debug] pose: {pose_path} (object={object_name})")
        print(f"[SixDPose debug] camera: {camera_path}")
        print(f"[SixDPose debug] depth: {depth_path}")
    ...
    def __getitem__(self, idx):
        ...
        if self.debug:
            self._debug_print_sample_paths(record)
